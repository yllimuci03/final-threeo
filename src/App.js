...
const socket = io.connect(' https://final-threeo.herokuapp.com');
  ...
  let storage=[...new Set(currentGameStorageOnline)]
    ...
    if (brownChipsOnline.length === 0 && placingCounterOnline === 1 && !isYellowThreeoOnline) {
      if (spot.classList.contains('yellow')) {
       ...
       socket.emit('placeYellowMoving', {id:id,room:roomName, game:gameOnline, yourTurn:true})
     }else if(local&&!connected){
       socket.emit('placeYellowMoving', {id:id,game:gameOnline,})
    ...
    if (brownChipsOnline.length === 0 && placingCounterOnline === 3 && !isBrownThreeoOnline) {
      ...
      if(yourTurnOnline&&connected&&!local){
       ...
       socket.emit('placeBrownMoving', {id:id, room:roomName, game:gameOnline, yourTurn:true})
     }else if(local&&!connected){
       socket.emit('placeBrownMoving', {id:id,game:gameOnline,})
